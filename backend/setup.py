...
def get_setup_status():
    ...
    # 3. Key Check
    status["has_gemini_key"] = bool(os.getenv("GEMINI_API_KEY"))
...
def save_api_key(request: KeyRequest):
    ...
    set_key(ENV_PATH, "GEMINI_API_KEY", request.key)
    
    # Update current process
    os.environ["GEMINI_API_KEY"] = request.key
    
    # Update the gateway singleton if it's already instantiated
    try:
        from gateway import get_gateway
        get_gateway().gemini_key = request.key
